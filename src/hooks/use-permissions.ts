 ...
 * if (hasAnyPermission([PERMISSIONS.USERS_EDIT, PERMISSIONS.USERS_DELETE])) { ... }
 * 
 * // Complex access check
 * if (canAccess({ permissions: [PERMISSIONS.USERS_VIEW], roles: [ROLES.ADMIN] })) { ... }
 */
...
/**
 * Convenience hook to check if user can access based on requirements
 * 
 * @example
 * const canManageUsers = useCanAccess({ 
 *   permissions: [PERMISSIONS.USERS_CREATE, PERMISSIONS.USERS_EDIT] 
 * })
 */
